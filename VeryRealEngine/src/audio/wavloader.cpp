...
WavLoader::WavLoader(const WavLoader &)
{
}

WavLoader &WavLoader::operator=(const WavLoader &)
{
	return (*this);
}
...
bool WavLoader::load(const char *path, WavClip *out_clip)
{
	FILE			*file;
	long			file_size;
	size_t			read_count;
	...
	{
		std::fprintf(stderr,
			"load_wav_file: \"%s\" is too small to be a WAVE file\n", path);
		std::fclose(file);
		return (false);
	}
	std::vector<uint8_t> bytes(static_cast<size_t>(file_size));
	read_count = std::fread(bytes.data(), 1, bytes.size(), file);
	std::fclose(file);
	...
	if (std::memcmp(bytes.data(), "RIFF", 4) != 0 || std::memcmp(bytes.data()
			+ 8, "WAVE", 4) != 0)
	{
	...
	while (offset + 8 <= bytes.size())
	{
		...
		chunk_size = read_le<uint32_t>(bytes.data(), offset + 4);
		chunk_data_offset = offset + 8;
		if (chunk_data_offset + chunk_size > bytes.size())
		{
			std::fprintf(stderr,
				"load_wav_file: \"%s\" has a truncated \"%s\" chunk\n", path,
				chunk_id);
			return (false);
		}
		if (std::strcmp(chunk_id, "fmt ") == 0 && chunk_size >= 16)
		{
			audio_format = read_le<uint16_t>(bytes.data(), chunk_data_offset
					+ 0);
			channels = read_le<uint16_t>(bytes.data(), chunk_data_offset + 2);
			sample_rate = read_le<uint32_t>(bytes.data(), chunk_data_offset
					+ 4);
			bits_per_sample = read_le<uint16_t>(bytes.data(), chunk_data_offset
					+ 14);
		}
	...
	{
		std::fprintf(stderr,
			"load_wav_file: \"%s\" is missing a \"fmt \" or \"data\" chunk\n",
			path);
		return (false);
	}
	...
	{
		std::fprintf(stderr,
			"load_wav_file: \"%s\" uses compressed audio format %u, only "
			"uncompressed PCM (format 1) is supported\n",
			path,
			audio_format);
		return (false);
	}
	...
	{
		std::fprintf(stderr,
			"load_wav_file: \"%s\" has an unsupported channel count (%u) "
			"or bit depth (%u) — only 8-bit or 16-bit PCM is supported\n",
			path,
						channels,
						bits_per_sample);
		return (false);
	...
	{
		sample_count = pcm_data_size / sizeof(int16_t);
		clip.samples().resize(sample_count);
		std::memcpy(clip.samples().data(), pcm_data, sample_count
			* sizeof(int16_t));
	}
	else // 8-bit PCM is unsigned, centered on 128 — up-convert to signed 16-bit.
	{
		...
		{
			centered = static_cast<int16_t>(pcm_data[i]) - 128;
