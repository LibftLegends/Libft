 ...
 * interface (ALSA on Linux; see src/platform/linux/audiolinux.hpp), plus a
 ...
 * hand-written WAVE loader (wavloader.hpp) — no third-party audio library.
 */
#pragma once
 ...
 *
 * The actual mixing happens off the render thread (see audiolinux.cpp's
...
class AudioSystem
{
  public:
	...
	/**
		...
		* @return A handle to pass to play(), or SoundHandle::invalid() on
		* failure (missing file, unsupported codec — see wavloader.hpp).
		*/
	...
	/**
		...
		* VoiceHandle::invalid() if `sound` is invalid or every voice slot is busy.
		*/
	virtual VoiceHandle play(SoundHandle sound, bool loop = false,
			float volume = 1.0f) = 0;
	...
	/// @return A newly constructed audio backend appropriate for the
	/// current platform.
