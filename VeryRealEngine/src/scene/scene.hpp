...
class Scene
{
	public:
		...
		/**
		 * @brief Parses `path` and loads every referenced mesh through
		 * `renderer`.
		 *
		 ...
		 * @param physics_world If non-null, rigid bodies are created here
		 * for nodes with a "physics" field.
		 * @return true on success.
		 */
		bool load(const char *path, Renderer *renderer,
		 ...
		 *
		 * Recomputes WorldTransformComponent for every entity in load
		 * order (parents before children, see load()'s doc comment
		 * above), then emits a RenderItem for every entity whose whole
		 * ancestor chain (including itself) is visible and that carries
		 * a MeshComponent. Delegates the actual algorithm to
		...
		/// @return This entity's own visibility flag (not combined with
		/// ancestors), or false if not found.
		 ...
		 * Overrides whatever static value the JSON scene file set;
		 * there's no way back to the JSON value short of reloading the
		 * scene, which is fine for a demo that only ever moves forward
		 * from here.
		 * @return false if no entity has that name.
		 */
		bool set_node_rotation(const std::string &name,
			const vec3 &euler_radians);
		/**
		 * @brief Looks up an entity's current world-space position by
		 * name.
		 *
		 * Used for the house demo's proximity checks (light switch/door).
		 * @param name Entity name to look up.
		 * @param out_position Receives the world-space position on
		 * success.
		 * @return false if no entity has that name.
		 */
		bool get_node_position(const std::string &name,
		 ...
		 * caster by Renderer::draw_frame() and must be a Directional
		 * light.
		 */
		 ...
		 *
		 * Addressed by index into get_lights() (scene lights have no
		 * name in the JSON format; the demo just needs to know which
		 * index it authored the switchable light at).
	...
	private:
		...
		float _ambient;
		TransformComposer _transform_composer;
		SceneLightParser _light_parser;
		SceneObjectLoader _object_loader;
		SceneRenderCollector _render_collector;
};
