/**
 * @file loader.hpp
 * @brief Minimal Wavefront .OBJ loader: opens the file, feeds each line to
 * an ObjGeometryBuilder, and hands back the assembled MeshData.
 *
 ...
 * `mtllib` diffuse color / diffuse texture map (see mtlloader.hpp).
 */
#pragma once
...
class ObjLoader
{
  public:
	...
	/**
		* @brief Loads `path` into `out_mesh` and appends any materials
		* referenced via `mtllib` to `out_materials`.
		*
		...
		* @param out_materials Materials referenced by the file are
		* appended here.
		* @return (true on success); false (leaving *out_mesh partially
		* filled) on parse/IO failure.
		*/
