...
void GeometryPass::create_render_pass(VkDevice device, VkFormat depth_format)
{
    ...
    scene_color_attachment.format = kSceneColorFormat;
    ...
    VK_CHECK(vkCreateRenderPass(device, &render_pass_info, nullptr, &_render_pass));
}

void GeometryPass::create_global_set_layout(VkDevice device)
{
    ...
    shadow_sampler_binding.descriptorCount = ShadowPass::kMaxShadowCasters;
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(device, &global_layout_info, nullptr, &_global_set_layout));
}
