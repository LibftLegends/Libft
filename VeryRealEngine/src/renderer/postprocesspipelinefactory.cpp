#include "postprocesspipelinefactory.hpp"
#include "vkcheck.hpp"
#include "vulkandevice.hpp"

namespace vre
{
PostProcessPipelineFactory::PostProcessPipelineFactory()
{
}

PostProcessPipelineFactory::PostProcessPipelineFactory(
	const PostProcessPipelineFactory &)
{
}

PostProcessPipelineFactory &PostProcessPipelineFactory::operator=(
	const PostProcessPipelineFactory &)
{
	return (*this);
}
...
PostProcessPipelineFactory::Resources PostProcessPipelineFactory::create(
	...
	size_t push_constants_size)
{
	...
	present_dependency.srcStageMask =
		VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
	present_dependency.srcAccessMask = 0;
	present_dependency.dstStageMask =
		VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
	...
	depth_sampler_binding.descriptorType =
		VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	...
	post_alloc_info.descriptorPool = resources.descriptor_pool;
	post_alloc_info.descriptorSetCount = 1;
	post_alloc_info.pSetLayouts = &resources.set_layout;
	VK_CHECK(vkAllocateDescriptorSets(device, &post_alloc_info,
			&resources.descriptor_set));

	VkShaderModule					vertex_module;
	VkShaderModule					fragment_module;
	VkDynamicState					post_dynamic_states[] = {
	...
	post_pipeline_info.layout = resources.pipeline_layout;
	post_pipeline_info.renderPass = resources.render_pass;
	post_pipeline_info.subpass = 0;
	VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1,
			&post_pipeline_info, nullptr, &resources.pipeline));
