...
void GeometryPass::create_global_ubo_resources(const VulkanDevice &device,
	const ShadowPass &shadow_pass, uint32_t frames_in_flight)
{
	...
	for (uint32_t i = 0; i < frames_in_flight; i++)
	{
		...
		vkMapMemory(vk_device, _ubo_memories[i], 0, buffer_size, 0,
			&_ubo_mapped[i]);
	}
	...
	VK_CHECK(vkCreateDescriptorPool(vk_device, &pool_info, nullptr,
	...
	_global_descriptor_sets.resize(frames_in_flight);
	VK_CHECK(vkAllocateDescriptorSets(vk_device, &alloc_info,
			_global_descriptor_sets.data()));
	for (uint32_t i = 0; i < frames_in_flight; i++)
	{
	...
	const std::vector<mat4> &bone_matrices)
{
	...
	for (uint32_t i = 0; i < light_count; i++)
	{
		...
		ubo.light_direction_or_position[i][3] = (light.type() == Light::Type::Point) ? 1.0f : 0.0f;
