 ...
 * @brief Picks a physical device, owns the logical VkDevice/queues/command
 * pool, and hosts the buffer/image utility helpers shared by every render pass.
 */
#pragma once
...
class VulkanDevice
{
  public:
	...
	/** @return An image view over `image`, shared by every render pass that needs one. */
	static VkImageView create_image_view(VkDevice device, VkImage image,
		VkFormat format, VkImageAspectFlags aspect_flags);
	/** @return A shader module loaded from a precompiled SPIR-V `.spv` file, shared by every pass. */
