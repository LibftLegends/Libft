...
{
  public:
	/// Hard cap on how many occlusion queries one frame-in-flight's
	/// pool can hold.
	 ...
	 * (see OcclusionPipelineFactory for why that's safe).
	 * @param frames_in_flight One query pool is created per
	...
	/// @return Whether `occlusion_id`'s last known query result said
	/// "produced zero visible samples."
	...
	/// requirement for vkCmdResetQueryPool), before the geometry render
	/// pass begins.
	 ...
	 * @param command_buffer Command buffer already inside the geometry
	 * render pass.
	 * @param frame_index Which frame-in-flight slot's query pool to
	 * record into.
	 * @param occlusion_test_items Items to test (already
	 * frustum-visible, drawn-for-real items excluded).
	 * @param mesh_registry Source of vertex/index buffers for the tested items.
	 * @param texture_registry Source of a valid material descriptor set
	 * to bind (see the .cpp's comment).
	 * @param geometry_pipeline_layout Pipeline layout reused verbatim
	 * from the geometry pass.
	 * @param global_descriptor_set Set 1 (globals), bound the same as
	 * the geometry pass.
