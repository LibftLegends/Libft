...
class MeshRegistry
{
  public:
	...
	/// @param texture_registry Used to create per-submesh materials
	/// during upload; must outlive this object.
	...
	/// placeholder cube (logging to stderr) if the file is missing or
	/// malformed.
	MeshHandle load_mesh_from_obj(const char *path);
	/// Loads a skinned mesh/skeleton/animation from a custom asset file.
	/// Falls back to a placeholder cube (with an empty skeleton/clip) on
	/// failure.
  ...
  private:
	...
	/// same treatment as GeometryPass::GlobalUbo, not a fully
	/// encapsulated class.
	struct				GpuSubMesh
	{
