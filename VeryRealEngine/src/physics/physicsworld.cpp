...
BodyHandle PhysicsWorld::add_body(const RigidBodyDesc &desc)
{
	...
	body.inverse_mass = (desc.is_static() || desc.mass() <= 0.0f) ? 0.0f
		: (1.0f / desc.mass());
...
void PhysicsWorld::detect_and_resolve()
{
	...
	for (size_t i = 0; i < _bodies.size(); i++)
	{
		...
		{
			PhysicsBody &a = _bodies[i];
			PhysicsBody &b = _bodies[j];
			...
			if (!CollisionDetector::aabb_overlap(
					CollisionDetector::compute_aabb(a.position, a.collider),
					CollisionDetector::compute_aabb(b.position, b.collider)))
				continue ;
			// Narrow phase: exact box/sphere test with normal + penetration.
			contact = CollisionDetector::collide(a.position, a.collider,
					b.position, b.collider);
			if (!contact.valid)
				continue ;
			...
			ContactResolver::resolve(&a, &b, contact);
		}
	}
