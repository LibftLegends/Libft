#include "audiolinux.hpp"

namespace vre
{
AudioLinux::AudioLinux() : _pcm_handle(nullptr), _running(false),
	_sample_rate(44100), _period_frames(0)
{
}
...
SoundHandle AudioLinux::load_sound(const char *path)
{
	return (_mixer.load_sound(path));
}

VoiceHandle AudioLinux::play(SoundHandle sound, bool loop, float volume)
{
	return (_mixer.play(sound, loop, volume));
}

void AudioLinux::stop(VoiceHandle voice)
{
	_mixer.stop(voice);
}

void AudioLinux::stop_all()
{
	_mixer.stop_all();
}

void AudioLinux::set_master_volume(float volume)
{
	_mixer.set_master_volume(volume);
}
...
bool AudioLinux::initialize()
{
	...
	result = snd_pcm_open(&_pcm_handle, "default", SND_PCM_STREAM_PLAYBACK, 0);
	if (result < 0)
	{
		// Not fatal — see audiosystem.hpp's initialize() doc comment: no
		// audio device (a headless CI box, a sandboxed session with no
		// sound permission, ...) means the engine runs silently, not that
		// it aborts.
		std::fprintf(stderr,
			"AudioLinux: snd_pcm_open failed: %s (running without audio)\n",
			snd_strerror(result));
		_pcm_handle = nullptr;
		return (false);
	}
	...
	snd_pcm_hw_params_set_access(_pcm_handle, hw_params,
	...
	// scheduling burden.
	period_frames = _sample_rate / 50;
	snd_pcm_hw_params_set_period_size_near(_pcm_handle, hw_params,
		&period_frames, nullptr);
	result = snd_pcm_hw_params(_pcm_handle, hw_params);
	if (result < 0)
	{
		std::fprintf(stderr,
			"AudioLinux: snd_pcm_hw_params failed: %s (running without "
			"audio)\n", snd_strerror(result));
	...
	{
		std::fprintf(stderr,
			"AudioLinux: snd_pcm_prepare failed: %s (running without "
			"audio)\n", snd_strerror(result));
		snd_pcm_close(_pcm_handle);
		_pcm_handle = nullptr;
		return (false);
...
void AudioLinux::writer_thread_main()
{
	...
	while (_running.load())
	{
		...
		written = snd_pcm_writei(_pcm_handle, period_buffer.data(),
				_period_frames);
		if (written < 0)
		{
			...
			written = snd_pcm_recover(_pcm_handle, static_cast<int>(written),
					0);
			if (written < 0)
			{
				std::fprintf(stderr,
					"AudioLinux: snd_pcm_writei unrecoverable: %s\n",
