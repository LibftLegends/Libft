...
class Window
{
  public:
	...
	enum class Key
	{
		...
		F,
		Count,
			///< Not a real key — array-sizing sentinel for per-key state tables.
	};
	...
	/**
		...
		*/
	virtual bool initialize(const char *title, int32_t width,
			int32_t height) = 0;
	...
	/**
		...
		*/
	virtual bool was_key_pressed(Key key) const = 0;
	...
	/**
		...
		*/
	virtual bool is_key_held(Key key) const = 0;
	...
	/**
		...
		*/
	virtual void get_required_instance_extensions(const char **out_extensions,
