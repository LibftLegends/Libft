/**
 * @file value.hpp
 * @brief Minimal JSON DOM. Paired with jsonparser.hpp/.cpp.
 *
 * Written from scratch — same "no non-system, no FullLibft" reasoning as
 * objloader/tgaloader (see verdict.md's dependency audit): FullLibft's
...
{
/**
 * @brief Which JSON value kind a JsonValue currently holds.
 *
 * Named `Boolean`, not `Bool` — X11's Xlib.h `#define`s `Bool` to `int`,
 * and that macro would rewrite this enumerator before the compiler ever
 * sees it, since `vre.hpp` pulls in X11 headers on Linux.
 */
enum class JsonType
{
	Null,
...
class JsonValue
{
  public:
	...
	/// @return This value's array elements (meaningful only when
	/// is_array() is true).
	const std::vector<JsonValue> &array_elements() const;

	/// @return This value's raw string payload (meaningful only when
	/// type() is JsonType::String).
	const std::string &string_value() const;

	void set_bool_value(bool value);
	void set_number_value(double value);
	void set_string_value(const std::string &value);
	/// Appends `value` to this array value (type() must already be
	/// JsonType::Array).
	void push_array_element(const JsonValue &value);
	/// Inserts or replaces `key` in this object value (type() must
	/// already be JsonType::Object).
