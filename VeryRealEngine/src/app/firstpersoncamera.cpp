...
FirstPersonCamera::FirstPersonCamera(const FirstPersonCamera &other) :
	_position(other._position), _yaw(other._yaw), _pitch(other._pitch),
...
FirstPersonCamera &FirstPersonCamera::operator=(
	const FirstPersonCamera &other)
{
	if (this != &other)
	{
...
mat4 FirstPersonCamera::view_matrix() const
{
	...
	return (mat4::look_at(_position, _position + look_forward,
			vec3(0.0f, 1.0f, 0.0f)));
}
