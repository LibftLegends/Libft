...
bool TgaLoader::load(const char *path, ImageData *out_image)
{
	...
	if (!file.is_open())
	{
		std::fprintf(stderr, "tgaloader: failed to open \"%s\"\n", path);
		return (false);
	}
	...
	{
		std::fprintf(stderr,
			"tgaloader: \"%s\" is too short for a TGA header\n", path);
		return (false);
	}
	...
	{
		std::fprintf(stderr,
			"tgaloader: \"%s\" uses unsupported image type %u (only "
			"uncompressed true-color TGA, type 2, is supported)\n", path,
			header.image_type);
		return (false);
	}
	...
	{
		std::fprintf(stderr,
			"tgaloader: \"%s\" has unsupported pixel depth %u (need 24 or "
	...
	{
		uint32_t color_map_bytes = header.color_map_length
			* (header.color_map_entry_size / 8);
	...
	if (!file)
	{
		std::fprintf(stderr, "tgaloader: \"%s\" is truncated\n", path);
		return (false);
	}
	...
	top_to_bottom = (header.image_descriptor & 0x20) != 0;
	...
	for (uint32_t y = 0; y < height; y++)
	{
		...
		const uint8_t *source = raw.data()
			+ static_cast<size_t>(source_row) * width * bytes_per_pixel;
		uint8_t *dest = out_image->pixels().data()
		...
		for (uint32_t x = 0; x < width; x++)
		{
			...
			uint8_t a = (bytes_per_pixel == 4)
				? source[x * bytes_per_pixel + 3] : 255;
