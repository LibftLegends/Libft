...
FT_TEST(test_voxel_runtime_block_registration_loads_assets)
{
...
FT_TEST(test_voxel_runtime_block_registration_rejects_missing_asset)
{
...
FT_TEST(test_voxel_runtime_block_handle_survives_unregister)
{
    voxel_block_registration registration;
    voxel_runtime_block_handle block_handle;
    uint32_t block_id;
    ft_size_t asset_size;
    const uint8_t *asset_data;
    uint32_t face_index;
    ...
    FT_ASSERT_EQ(FT_ERR_SUCCESS, voxel_register_block(registration,
        &block_id));
    FT_ASSERT_EQ(FT_ERR_SUCCESS, voxel_acquire_block(block_id,
        block_handle));
    FT_ASSERT_EQ(FT_TRUE, block_handle.is_valid());
    FT_ASSERT_EQ(block_id, block_handle.get_id());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, voxel_unregister_block(block_id));
    FT_ASSERT_EQ(FT_FALSE, voxel_block_is_known(block_id));
    FT_ASSERT_EQ(FT_TRUE, block_handle.is_valid());
