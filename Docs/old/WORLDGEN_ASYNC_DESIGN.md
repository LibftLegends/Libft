...
Implementation status: implemented in the Minecraft branch. This document is being archived after verification; the code is now the authoritative behavior.
...
- `src/world/WorldGenerationPipeline.hpp/.cpp`: bounded worker pool, owned requests/results, cancellation epochs, generation-priority queueing, deferred cross-chunk writes, immutable snapshots, and worker remeshing.
- `src/world/World.cpp`: asynchronous streaming submission, main-thread result publication, stale-result checks, dirty-neighbor remesh scheduling, deferred-edit application, and asynchronous revision regeneration.
- `src/chunks/WorldChunk.hpp/.cpp`: voxel revisions, pending mesh request identity, and dirty-mesh state.
- `src/chunks/WorldChunkLoader.cpp` and edit commands: synchronous paths invalidate async mesh results through the voxel revision.
- `src/validators/WorldAsyncGenerationValidator.cpp`: async/synchronous terrain parity and lifecycle coverage, exposed through `--validate-async-generation` and included by `make test`.

The shipped defaults are automatic worker count capped at four, a bounded generation queue, a two-result/approximately-two-millisecond main-thread drain, and at most one in-flight border remesh. A large explicit generation budget retains the existing synchronous path for startup/validation compatibility. Normal gameplay budgets use the asynchronous path. The revision server starts regeneration asynchronously; the legacy `regenerate_selected_chunks()` API remains a blocking compatibility wrapper for existing callers and validators.

The design's optional future tuning surface (runtime-configurable worker count and queue/commit limits) is intentionally represented by the pipeline defaults for this first implementation; changing those defaults does not alter ownership or synchronization rules.
