...
Status: implemented; retained in `Docs/old` as the historical design record
...
The applicable low-risk work from this design was implemented on the Minecraft
optimization branch:

- hidden debug collection is gated before RAM, VRAM, biome, and revision work;
- system-memory and approximate-VRAM diagnostics are refreshed at a slower
  cadence, and revision-preview storage is reused and refreshed conditionally;
- unchanged GPU dimensions skip resize work, with the world pass retaining the
  single authoritative viewport setup;
- visibility collection uses a precomputed conservative six-plane frustum and
  positive-vertex AABB tests instead of transforming eight corners per chunk;
- generation snapshots use Libft bulk block and border-copy APIs;
- deferred edits return immediately when empty and share the generation-result
  time deadline instead of running outside the frame budget;
- static menus cache their rasterized canvas and upload only after invalidation,
  while loading animation remains continuously invalidated;
- the performance harness reports p95 and p99 frame times in addition to FPS;
- Windows event-loop wakeups retain their pipe-based interrupt behavior while
  using the platform-correct descriptor functions.

The inactive mega-buffer path and cross-frame visibility caching were not
enabled: neither has sufficient correctness/performance evidence, and the
design explicitly requires measurement before introducing those higher-risk
changes. Software-renderer parallelization and GPU timer queries likewise
remain profiling follow-ups rather than unverified runtime changes.
