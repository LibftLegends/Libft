    ...
    cross_process_read_result &result)
{
    ...
    ft_string staged_shared_memory_name;
    ft_string staged_payload;
    ...
    operation_error = staged_shared_memory_name.initialize();
    if (operation_error != FT_ERR_SUCCESS)
        return (cleanup_and_fail(FT_ERR_NO_MEMORY));
    operation_error = staged_payload.initialize();
    if (operation_error != FT_ERR_SUCCESS)
        return (cleanup_and_fail(FT_ERR_NO_MEMORY));
    assign_error = staged_shared_memory_name.assign(
        message.shared_memory_name,
        ft_strlen_size_t(message.shared_memory_name));
    ...
    assign_error = staged_payload.assign(reinterpret_cast<char *>(
    ...
    operation_error = result.shared_memory_name.move(staged_shared_memory_name);
    if (operation_error != FT_ERR_SUCCESS)
    {
        cleanup_error = cmp_cross_process_unlock_mutex(message, &mapping,
                &mutex_state);
    ...
    operation_error = result.payload.move(staged_payload);
    if (operation_error != FT_ERR_SUCCESS)
    {
        cleanup_error = cmp_cross_process_unlock_mutex(message, &mapping,
                &mutex_state);
