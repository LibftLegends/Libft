...
static const uint32_t TERRAIN_SAVE_VERSION = 11U;
    ...
    terrain_generation_config &config, ft_byte_buffer &buffer) noexcept
{
    ...
    if (error_code != FT_ERR_SUCCESS
        || (version != TERRAIN_SAVE_VERSION && version != 10U && version != 9U
    ...
    if (version >= 11U)
    {
        ...
        if (error_code != FT_ERR_SUCCESS)
            return (error_code);
        error_code = terrain_save_read_i32(buffer, &underground_lake_minimum_y);
        if (error_code != FT_ERR_SUCCESS)
            return (error_code);
        error_code = terrain_save_read_i32(buffer, &underground_lake_maximum_y);
        if (error_code != FT_ERR_SUCCESS)
            return (error_code);
